...
export default function TestInvitationPage() {
    ...
    const t = setTimeout(async () => {
      try {
        const res = await fetch("/api/admin/test-invitation", {
          ...
          body: JSON.stringify({ preview: true, messageType, guestName, plusOneName }),
        });
  ...
  }, [messageType, guestName, plusOneName]);
  ...
  const doSend = async () => {
    ...
    try {
      const res = await fetch("/api/admin/test-invitation", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ phone, messageType, guestName, plusOneName, sessionId }),
      });
      const data = await res.json();
      if (!res.ok) {
      ...
      setOkMsg(`✓ Pesan undangan terkirim ke ${data.phone}. Cek WhatsApp Anda.`);
      setStep("done");
    } catch {
  ...
  // Gate behind OTP verification of the sender number (same as the other sends).
  ...
  return (
    <div className="max-w-xl">
      ...
      <p className="text-sm text-gray-500 mb-6">
        Kirim pesan undangan ke nomor WhatsApp Anda sendiri untuk melihat tampilannya sebelum dikirim ke tamu.
      </p>

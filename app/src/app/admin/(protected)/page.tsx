...
export default async function AdminDashboard() {
  ...
  const cards = [
    { label: "Total RSVPs", value: stats.total, color: "bg-blue-50 text-blue-700", icon: "📋" },
    { label: "Attending", value: stats.attending, color: "bg-green-50 text-green-700", icon: "🎉" },
    { label: "Declined", value: stats.declined, color: "bg-red-50 text-red-700", icon: "💌" },
    { label: "Pending", value: stats.pending, color: "bg-yellow-50 text-yellow-700", icon: "⏳" },
    { label: "Checked In", value: stats.checkedIn, color: "bg-[var(--color-cream-dark)] text-[var(--color-gold)]", icon: "✅" },
  ];
    ...
    <div>
      ...
      {stats.attending > 0 && (
        <div className="bg-white rounded-xl p-5 mb-6 shadow-sm">
          <p className="text-sm font-medium text-gray-700 mb-3">
            Attendance Rate — {Math.round((stats.attending / (stats.attending + stats.declined || 1)) * 100)}%
          </p>
      ...
      {stats.attending > 0 && (
        <div className="bg-white rounded-xl p-5 shadow-sm">
          <p className="text-sm font-medium text-gray-700 mb-3">
            Check-in Progress — {Math.round((stats.checkedIn / (stats.attending || 1)) * 100)}%
          </p>
          <div className="h-3 bg-gray-100 rounded-full overflow-hidden">
            <div
          ...
          <div className="flex justify-between text-xs text-gray-400 mt-1">
            ...
            <span>{stats.attending - stats.checkedIn} not yet arrived</span>
          </div>
        </div>
      )}
